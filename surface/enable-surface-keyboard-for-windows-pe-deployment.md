...
description: 當您使用 MDT 將 Windows 10 Surface 膝上型電腦部署時，您需要將鍵盤驅動程式導入到 Windows 環境中使用。
...
ms.locfileid: "11912008"
---
# <a name="how-to-enable-the-surface-laptop-keyboard-during-mdt-deployment"></a>如何在 MDT 部署期間Surface Laptop鍵盤

本文說明使用 Microsoft Deployment Toolkit (MDT) 。 您也可以將這項資訊用於其他部署方法。 在大部分類型的 Surface 裝置上，鍵盤在 Lite Touch 安裝或 LTI (期間) 。 不過，Surface Laptop一些額外的驅動程式才能啟用鍵盤。 針對 Surface Laptop (第 1 代) 和 Surface Laptop 2 個裝置，您必須準備資料夾結構和選取設定檔，讓您指定鍵盤驅動程式，以在 LTI Windows 安裝前環境 (Windows PE) 階段使用。 有關此資料夾結構的資訊，請參閱使用[MDT 部署Windows 10圖像：步驟 5：準備驅動程式存放庫](/windows/deployment/deploy-windows-mdt/deploy-a-windows-10-image-using-mdt?redirectedfrom=MSDN#step-5-prepare-the-drivers-repository)。
...
> [!IMPORTANT]
> 如果您要將 Windows 10 映射部署到預先安裝 S 模式 Windows 10 Surface Laptop 的 Surface Laptop，請參閱 KB 4032347，將 Windows 部署到在 S 模式中預先安裝 Windows 10 的 Surface 裝置時[發生問題](https://support.microsoft.com/help/4032347/surface-preinstall-windows10-s-mode-issues)。
...
4. 使用選取設定檔或**DriverGroup001**變數，Surface Laptop其餘的驅動程式。
    - 如果Surface Laptop (第 1 代) ，則**模型會**Surface Laptop。 其餘的 Surface Laptop驅動程式應該位於 \MDT 部署共用\開箱驅動程式\Windows10\X64\Surface Laptop 資料夾中，如下圖所示。
