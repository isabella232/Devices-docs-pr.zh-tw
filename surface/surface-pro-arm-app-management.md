...
ms.locfileid: "12449596"
---
# <a name="deploying-managing--servicing-surface-pro-x"></a>部署、管理及維護 Surface Pro X

Surface Pro X 專為滿足高效能商業需求而打造，結合了該等級中空前強大的處理器 Microsoft SQ1 以及 Microsoft SQ1 ARM 晶片組。

Surface Pro X 由 3GHz CPU 與 2.1 teraflop GPU 提供支援，讓您享有完整的 Windows 體驗。 其電池使用時間長達 15 小時並且內建 Gigabit LTE，集觸控、手寫筆、平板電腦、和膝上型電腦為一身，非常適合金融、法律和醫學等領域的各類前線工作者及專業人員，或任何需要超長電池使用時間和持續連線功能的職務角色。
...
Autopilot 部署有幾個優點：允許您使用針對全自動部署所簡化的原廠佈建作業系統，將預先安裝的 [Microsoft 365 Apps 企業版](https://www.microsoft.com/microsoft-365/enterprise/microsoft-365-apps-for-enterprise)納入其中。

已經使用現代化管理、安全性及生產力解決方案的組織具備得天獨厚的優勢，可以利用 Surface Pro X 中特有的效能功能。使用現代化[商務營運應用程式](/microsoft-store/working-with-line-of-business-apps)、[Microsoft Store (UWP) 應用程式](/windows/uwp/get-started/universal-application-platform-guide)或遠端桌面解決方案的客戶也將受益匪淺。

## <a name="image-based-deployment-considerations"></a>映像式部署考量

Microsoft Deployment Toolkit (MDT) 和 Microsoft Endpoint Configuration Manager (舊稱 System Center Configuration Manager) 不支援 Surface Pro X 進行作業系統部署。 在持續評估轉換至現代部署解決方案適當時機的期間，倚賴映像式部署的客戶不妨考慮 Surface Pro 8。 

## <a name="managing-surface-pro-x-devices"></a>管理 Surface Pro X 裝置

### <a name="intune"></a>Intune

[Microsoft Intune](/intune) 與 Azure Active Directory 整合以進行身分識別和存取控制，並提供對已註冊 Surface Pro X 裝置的細微管理。 Intune 行動裝置管理 (MDM) 原則設定与舊版内部部署工具 (例如 Windows 群組原則) 相比有几个優勢。 這包括更快速的裝置登入時間，以及更加簡化得可以完全從雲端管理裝置的原則設定資料目錄。 例如，您可以[使用 eSIM 設定檔管理 LTE](/windows/client-management/mdm/esim-enterprise-management) 以設定行動數據方案，並將啟動碼部署至多個裝置。

### <a name="co-management"></a>共同管理

使用 Autopilot 進行部署後，您可以將 Surface Pro X 裝置加入 Azure AD 或 Active Directory ([混合式 Azure AD Join](/azure/active-directory/devices/concept-azure-ad-join-hybrid))，在該處您可以[使用 Intune 來管理裝置](/mem/intune/remote-actions/device-management)，或是[使用 Endpoint Configuration Manager 來共同管理裝置](/mem/configmgr/comanage/overview) (將會安裝 32 位元 x86 ConfigMgr 用戶端)。
...
Microsoft Defender 將協助保護 ARM 型電腦上的 Windows 10 和 Windows 11，使其在裝置支援的生命週期內得到保護。 

部分協力廠商防毒軟體無法安裝在執行 ARM 型處理器的裝置上。 仍持續與協力廠商防毒軟體提供者進行合作，處理防毒應用程式在 ARM 型電腦上的整備工作。 請連絡防毒軟體提供者，以了解他們的應用程式何時會推出。
...
Surface Pro X 的設計目的是使用 Windows Update 簡化作業，使家庭使用者和小型企業使用者的驅動程式和韌體保持最新。 使用預設的設定來接收自動更新。  若要確認：
...
- 請注意，Windows Server Update Services (WSUS) 不支援傳遞驅動程式和韌體至 Surface Pro X。
...
- 使用為執行在 ARM 型處理器上的 Windows 10 或 Windows 11 電腦設計的驅動程式的應用程式。
- Windows 的 x64 模擬現於 Windows 11 中正式提供。
...
Azure 虛擬桌面可讓您在任何運算裝置或平台上，從任何位置存取 Windows 桌面、應用程式和資料。 若要深入了解，請參閱 [Azure 虛擬桌面網站](https://aka.ms/wvd)。 
...
- 以 Chromium 為基礎的 Firefox 和 Microsoft Edge 在 ARM 型 Windows 10 或 Windows 11 電腦上原生執行，效能提升。

## <a name="installing-and-using-microsoft-office"></a>安裝和使用 Microsoft Office

- 在執行 ARM 型處理器的 Windows 10 或 Windows 11 電腦上使用 Office 365 可獲得最佳體驗。
- Office 365「隨選即用」安裝 Outlook、Word、Excel 和 PowerPoint，並已經過最佳化，可在使用 ARM 型處理器的 Windows 10 或 Windows 11 電腦上執行。
...
下表顯示 Surface Pro X 在使用 ARM 的 Windows 10 或 Windows 11 使用時，可用的特選關鍵功能。
...
| Surface UEFI 設定程式                     | 否      | Surface Pro X 沒有在韌體層級停用硬體的選項。                |
...
| 支援協力廠商防毒軟體             | 請參閱附註| 部分協力廠商防毒軟體無法安裝在 ARM 型處理器上。 請連絡防毒軟體提供者，以了解他們的應用程式何時會推出。 |
| 安全開機                                   | 是     |                                                                                       |
| Windows 資訊保護                | 是     |                                                                                       |
...
Microsoft Deployment Toolkit (MDT) 和 Microsoft Endpoint Configuration Manager 目前不支援 Surface Pro X 進行作業系統部署。 依賴映像式部署的客戶應考慮 Surface Pro 8，同時繼續評估要轉換至雲端的合適時間。
