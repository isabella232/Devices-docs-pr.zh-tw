...
Surface UEFI 重設套件是從 SEMM 取消註冊 Surface 裝置的主要方法。 就像 Surface UEFI 設定套件一樣，重設套件是Windows安裝程式 (.msi) 檔案，在裝置上設定 SEMM。 與組組套件不同，重設套件會將 Surface 裝置上的 Surface UEFI 設定重設為預設設定、移除 SEMM 憑證，以及從 SEMM 取消註冊裝置。

重設套件是專為個別 Surface 裝置所建立。 若要開始建立重設套件程式，您需要要取消註冊的裝置序號，以及註冊裝置所使用的 SEMM 憑證。 您可以在 Surface UEFI 的 PC**** 資訊頁面上找到 Surface 裝置的序號，如圖 1 所示。 即使 Surface UEFI 受密碼保護且輸入的密碼不正確，此頁面也會顯示。
...
>[!NOTE]
>若要啟動至 Surface UEFI，在裝置**** 關閉時同時按**音量**增加和電源。 將 **音量保持為最高** ，直到 Surface 標誌顯示且裝置開始啟動。
...
在 Surface 裝置上Windows安裝程式 (.msi) Surface UEFI 重設套件，以從 SEMM 取消註冊裝置。 重設套件需要重新開機才能執行取消註冊作業。 取消註冊裝置之後，您可以確認不再顯示圖****) 5 (所示的 Microsoft Surface **Configuration Package**專案，以驗證移除是否成功。
...
在某些情況下，Surface UEFI 重設套件可能無法從 SEMM (取消啟用 Surface 裝置，例如，Windows已無法使用) 。 在這些情況下，您可以使用 Surface UEFI 中產生的修復要求來取消註冊裝置。 即使您沒有 Surface UEFI 密碼的裝置，也可以啟動修復要求程式。

復原要求程式是由 Surface 裝置上的 Surface UEFI 啟動，已由另一部電腦的 Microsoft Surface UEFI 組員核准，然後在 Surface UEFI 中完成。 與重設套件一樣，核准 Microsoft Surface UEFI 設定器的修復要求時，必須存取用於註冊 Surface 裝置之 SEMM 憑證。
...
5. 按一下或按 **[下一** 步， 以開始修復要求程式。
   >[!NOTE]
   >復原要求會在建立後的兩小時到期。 如果此時尚未完成修復要求，您必須重新啟動修復要求程式。
6. 從 **[選擇 SEMM** 重設金鑰 (顯示之憑證清單中選取 **[SEMM** 憑證) ，然後按一下或按 [下一 **步**> 。
...
7. 在 [輸入**SEMM**重設驗證碼> 頁面上，您可以**** 按一下**QR**碼或文字按鈕，以顯示覆原要求 (重設要求) ，如圖 8 所示，或 USB 按鈕將修復要求 (重設要求) 儲存為檔案至 USB 磁碟機，如圖 9 所示。 ****

   ![以 QR 程式碼顯示覆原要求。](images/surface-semm-unenroll-fig8.png "Recovery Request displayed as a QR Code")

   *圖 8. 復原要求 (重設) 顯示為 QR 程式碼*

   ![將復原要求儲存到 USB 磁碟機。](images/surface-semm-unenroll-fig9.png "Save a recovery request to a USB drive")

   *圖 9. 將復原要求儲存 (重設) USB 磁碟機*

   * 若要使用 QR 程式碼修復 (重設) ，請使用行動裝置上的 QR 閱讀程式應用程式來讀取程式碼。 QR 閱讀程式應用程式會將 QR 程式碼轉換成數位字串。 接著，您可以使用 Microsoft Surface UEFI 設定器，以電子郵件或訊息將字串以電子郵件或訊息給系統管理員，以產生重設驗證碼。
   * 若要使用儲存至 USB 磁碟機的復原要求 (重設要求) 檔案，請使用 USB 磁碟機將檔案傳輸至 Microsoft Surface UEFI 設定器用來產生重設驗證碼的電腦。 檔案也可以從 USB 磁碟機複製到另一個裝置上，以電子郵件傳送或網路傳輸。
...
14. 按 **\[下一步\]**。
15. 輸入 [復原要求 (重設) ，然後按一下 [產生以建立**** 重設驗證碼 (如圖 12) 。

    ![輸入復原要求。](images/surface-semm-unenroll-fig12.png "Enter the recovery request")

    *圖 12. 輸入復原要求 (重設要求) *

    * 如果您在要重設的 Surface 裝置上 (重設要求) 文字，請使用鍵盤在提供的欄位中輸入修復要求 (重設) 要求。
    * 如果您將修復要求 (重設要求) 顯示為 QR 程式碼，然後使用訊息或電子郵件應用程式將程式碼傳送至具有 Microsoft Surface UEFI 設定器的電腦，請複製程式碼並貼到提供的欄位中。
    * 如果您將 [復原要求 (重設要求) 儲存為 USB 磁碟機的檔案，請按一下 [**** 匯出按鈕，流覽至並選取 [復原要求 (重設要求) 檔案，然後按一下 [**確定**。
...
18. 按一下或按**SEMM** **** 重設成功頁面上的 [立即重新開機，以完成 SEMM 的取消註冊，如圖 14 所示。
