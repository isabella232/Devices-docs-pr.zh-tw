...
使用 Windows 10 小組版作業系統（在 Surface Hub 2：執行），只有獲授權的使用者才能使用 [設定] app 來調整設定。 由於選擇 [隸屬關係] 可能會影響功能可用性，請適當地規劃，以確保使用者可以依預期存取功能。

> [!NOTE]
> 您只能在初始全新體驗（OOBE）設定期間設定裝置附屬關係。 如果您需要重設裝置附屬關係，您必須重複 OOBE 設定。
...
當您選擇將 Surface Hub 2 與 Azure Active Directory （Azure AD）結合時，全域系統管理員安全性群組中的任何使用者都可以登入 Surface Hub 2 上的 [設定] 應用程式。 目前，不能委派其他群組登入 Surface Hub 2 上的 [設定] 應用程式。
