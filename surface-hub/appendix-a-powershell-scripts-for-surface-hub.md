...
-   [適用於 Surface Hub 系統管理員的 PowerShell 指令碼](#scripts-for-admins)
    ...
    -   [使用 Office 365 建立裝置帳戶](#create-os356-ps-scripts)
    -   [帳戶驗證指令碼](#acct-verification-ps-scripts)
...
- [適用於 IT 專業人員的 Microsoft Online Services 登入小幫手 RTW](https://www.microsoft.com/download/details.aspx?id=41950)
...
<td align="left"><p>&quot;這是 Surface Hub 會議室！&quot;</p></td>
</tr>
<tr class="odd">
...
<td align="left"><p>設定為裝置帳戶的使用者主體名稱 (UPN)</p></td>
</tr>
<tr class="odd">
<td align="left"><p>Set-MsolUserLicense (僅限 O365)</p></td>
<td align="left"><p>AddLicenses</p></td>
<td align="left"><p>使用者提供</p></td>
</tr>
<tr class="even">
<td align="left"><p>Set-MsolUser (僅限 O365)</p></td>
<td align="left"><p>PasswordNeverExpires</p></td>
<td align="left"><p>True</p></td>
</tr>
<tr class="odd">
<td align="left"><p>Set-AdUser (僅限內部部署)</p></td>
...
在下列 Cmdlet 中，`$strPolicy` 是 ActiveSync 原則的名稱，而 `$strRoomUpn` 是您想要套用原則的裝置帳戶的 UPN。

請注意，若要執行 Cmdlet，您需要設定遠端的 PowerShell 工作階段，而且：

-   您的系統管理員帳戶必須啟用遠端 PowerShell。 這讓系統管理員能夠使用指令碼所需的 PowerShell Cmdlet (您可以使用 `set-user $admin -RemotePowerShellEnabled $true` 來設定此權限)。
-   如果您打算執行建立指令碼，您的系統管理員帳戶就必須具備「重設密碼」角色。 這讓系統管理員能夠變更指令碼所需的帳戶密碼。 您可以使用 Exchange 系統管理中心來啟用 \[重設密碼\] 角色。
...
若要套用原則，信箱就不能是會議室類型，因此，必須先將它轉換為使用者。
