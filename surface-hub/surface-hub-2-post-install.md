...
8. 如果配對失敗，您可以嘗試再次配對筆。 如果仍無法解決問題，您可以在白板應用程式中驗證手寫筆是否正常運作，以查看電池是否已充電。 如果不是，請更換電池，然後再次嘗試配對筆。 如有需要，請重新開機裝置，然後再試一次。

**設定畫筆快速鍵** Surface Hub 筆有一個快速鍵按鈕，有時稱為「箭尾按一下」。 [設定快速鍵] 會要求您先對筆進行配對，如前文所述。

1. 搜尋手寫筆，然後選取 [ **畫筆] & Windows Ink 設定**。
...
> [!IMPORTANT]
> 在 Windows 10 版本2004及更新版本中，預設不會安裝使用 Miracast 進行無線投影的連線應用程式，但可將它做為選用的功能。 如果您已安裝 (或更新為) Windows 版本2004或更新版本，您可能會在 [設定] 中的 [投影至此電腦畫面] 畫面上看到下列資訊：

![投影到這台電腦](images/sh2-project.png) 


1. 若要從 [投影到此電腦] 設定頁面安裝應用程式，請選取 [**選用功能**]  >  **新增功能**，然後安裝 [**無線顯示**] 應用程式。

2. 在 **某些 Windows 和 Android 裝置下，當您說到 [確定] 時，可以將它投影到這台電腦**，請選擇：

    - 如果裝置不在商業網路上，就**可以使用所有位置**。
    - 否則，請選擇 [ **所有位置都能在安全網路上使用**]。
    
3. 在 [ **要求 project to 此電腦**] 底下，選擇 [ **僅限第一次**]。

4. 在 [ **需要 PIN 以進行配對**] 下，選擇 [ **永不**]。

5. 若要啟動 app 並將它釘選到工作列，請搜尋 **[連線]**。

6. 開啟 app。 在 app 開啟時，以滑鼠右鍵按一下工作列上的 [連接] 應用程式圖示，然後選取 [ **釘選到工作列**]。

7. 然後關閉 [連線] app。 除非 app 至少執行過一次，否則**此電腦的 Project**可能無法運作。
...
3. 展開 [ **電源管理**  ]，然後選取 [ **不要將 IT 部門的電源設定套用至此電腦**]。
...
6. 確認投影到此電腦的 [連線] App 已啟用。 **將 Project 設定為這台電腦**設定之後，請至少執行一次 [連線] App。  (接著，[連接] App 不需要執行，就能投影至 Surface Hub。 ) 

7. 驗證電源和睡眠設定。

    - 螢幕保護裝置程式：15分鐘，設定為 (無]) 、Mystify 或空白;確認已選取 [需要密碼的核取方塊]。
    - 螢幕： **在2小時後關閉**。
    - PC：  **4 小時後關閉**。
